...
def create_dataset(dataroot: str, phase: str, serial_batches: bool, preprocess: str, no_flip: bool, load_size: int,
    ...
    """
    dataset = UWIEDataset(dataroot=dataroot, phase=phase, serial_batches=serial_batches,
                          preprocess=preprocess, flip=not no_flip, load_size=load_size,
                          crop_size=crop_size)

    dataloader = UWIEDataloader(dataset, batch_size=batch_size, num_threads=num_threads,
                                serial_batches=serial_batches, is_distributed=is_distributed,
                                use_cuda=use_cuda)
